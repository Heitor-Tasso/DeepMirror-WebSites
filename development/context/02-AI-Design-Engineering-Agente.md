...
## O Input do Agente: A Pasta `/clean`

### 3.0 - O que o agente recebe

O agente de IA não lida com captura, desminificação ou reorganização de arquivos. Ele recebe uma pasta `/clean` já processada pelo DeepMirror WebSites com a seguinte estrutura padronizada:

```
clean/
├── index.html                        # HTML desminificado, paths corrigidos
├── [outras páginas].html
├── assets/
│   ├── css/
│   │   ├── styles.css                # CSS desminificado e consolidado
│   │   └── critical.css
│   ├── js/
│   │   └── main.js                   # JS limpo, sem tracking
│   ├── fonts/
│   │   └── *.woff2
│   ├── images/
│   │   └── *.webp, *.png, *.jpg
...
│       └── *.glb, *.gltf, *.hdr
└── _metadata/
    ├── extracted_tokens.json          # Variáveis CSS, paleta, fontes
    ├── computed_styles.json           # getComputedStyle dos elementos-chave
    ├── scroll_physics.json            # Mapeamento de scroll (se houver)
    └── path_mapping.json
```

### 3.1 - Os metadados são o verdadeiro input

Os arquivos em `_metadata/` são **mais valiosos para a IA do que o HTML/CSS** em si:

- **`extracted_tokens.json`**: Variáveis CSS, paleta de cores, fontes, espaçamentos — é o Design System já destilado em formato consumível
- **`computed_styles.json`**: Valores reais renderizados de cada elemento-chave — elimina ambiguidade do Cascade
- **`scroll_physics.json`**: Mapeamento de como elementos se comportam durante scroll — física exata para reprodução

O agente deve **sempre começar pelos metadados**, usar o HTML/CSS como referência visual de estrutura, e só consultar assets individuais quando precisar de detalhes específicos.
...
### 10.0 - Python + `uv`: Setup mínimo e rápido
...
### 10.1 - Estrutura do projeto do agente
...
### 10.2 - O fluxo de execução do agente
...
### 10.3 - Chamada à API da Anthropic (exemplo real)
...
def generate_section(section_name: str, context: dict, clean_path: Path) -> str:
    """Gera uma seção do design system via Claude."""
    
    # Carregar template de prompt
    template = load_template(f"{section_name}.jinja2")
    prompt = template.render(
        tokens=context["tokens"],
        fonts=context["fonts"],
        palette=context["palette"],
        computed_styles=context.get("computed_styles", {}),
        available_assets=context["assets"],
    )
    
    response = client.messages.create(
        model="claude-sonnet-4-20250514",
        max_tokens=8192,
        system=load_template("system_prompt.jinja2").render(context=context),
        messages=[
...
### 10.4 - Por que o output é JSX e não HTML puro
...
### 10.5 - Multi-turn e gestão de contexto
...
### 8.5 - Não validar o output contra os tokens de entrada
