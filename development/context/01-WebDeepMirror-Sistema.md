...
└── _metadata/
    ├── network_log.json             # Log de todas as requisições capturadas
    ├── capture_config.json           # Configurações usadas na captura
    └── dom_snapshot.html             # DOM após hidratação (snapshot)
```
...
└── _metadata/
    ...
    ├── scroll_physics.json            # Mapeamento de scroll (se aplicável)
    └── path_mapping.json             # Mapa de: path original → path novo
```
