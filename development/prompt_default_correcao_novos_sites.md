...
Sua função está na Manutenção e evolução do DeepMirror-WebSites — sistema de download de sites de alta fidelidade baseado em Runtime Network Recording. O projeto já passou por uma refatoração completa (monolítico para modular) e está funcional. Agora precisa de correções de bugs, melhorias em algumas funções para melhorar a extração dos sites e conseguir ter sucesso com mais sites complexos, não só com os já testados.

</role>

<project_context>

Projeto Python que baixa sites para visualização offline com alta fidelidade, capturando experiências complexas (WebGL, Three.js, Rive, SPAs) através de interceptação de rede em runtime.

**Propósito:** Os sites baixados são usados para AI Design / Vibe Design — ensinar IAs a programar código com o estilo visual de sites de referência. Por isso a fidelidade do download é crítica.
...
O sistema funciona em 3 pilares dentro do pacote `downloader/`:7

1. **browser.py**: Controla o Playwright, faz scroll e interações para "acordar" o site.
2. **network.py**: Intercepta requests (`page.on('response')`), salva arquivos em `assets/` e cria um mapa `{url_remota: path_local}`.
3. **post_process.py**: Lê os arquivos salvos e faz "Blind Replace" das URLs remotas pelos caminhos locais.
...
Antes de tudo, leia os 3 ultimos problemas resolvidos no changelog, mas veja que pode ter ficado alguns pontos a melhorar.
...
1. **Análise de Logs e Diagnóstico**:
   ...
   - Começe uma busca para encontrar onde que se encontra o problema:
      ...
      5) Gere um relatório final de tudo que encontrado
      6) Começe a executar tudo e finalize
      7) Reinicie o projeto e faça o download novamente e extraia.
      8) Abra os sites usando o Playwright e depois veja como está os logs do console e do python.
      9) Se não tiver resolvido, reinicie o processo. Se tiver deixa eu testar.
...
- NÃO crie arquivos auxiliares (scripts bash, arquivos de texto, configs extras, docs intermediários) sem o USUÁRIO pedir explicitamente;
...
- Rode TODOS os comandos necessários você mesmo (install, test, build). Não peça ao usuário para rodar manualmente
...
Para considerar o trabalho concluído, você deve validar se o código atende aos seguintes Casos de Uso:

### Caso 1: Prevenção de Corrupção de JS (SyntaxError)
**Cenário:** O `post_process.py` não deve mais tentar substituir strings cegamente dentro de arquivos `.js` complexos, o que causava `Unexpected token`.
**Como Validar:** Verifique no código se a função de reescrita (`_global_url_rewrite`) tem uma verificação explicita para pular ou tratar com extremo cuidado arquivos `.js`, delegando a resolução de URLs para o runtime (Interceptor).

### Caso 2: Resolução de Imports Dinâmicos (Erro 404)
**Cenário:** O interceptor deve lidar com chamadas como `import('./assets/chunk.js')`.
**Como Validar:** Verifique se o código injetado no HTML (`_inject_fetch_interceptor`) possui lógica para normalizar URLs relativas antes de buscar no mapa de recursos.
